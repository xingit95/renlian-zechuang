...
BOOL CFaceDetectDlg::OnInitDialog()
{
	...
	//打开图像
	m_btn_open.SubclassDlgItem(IDC_BTN_OPENFILE,this);
	m_btn_open.SetIcon(IDI_ICON_OPEN);

	
	//相似度计算
	m_btn_likehood.SubclassDlgItem(IDC_BTN_LIKEHOOD,this);
	m_btn_likehood.SetIcon(IDI_ICON_LIKEHOOD);

	//二值化
	m_btn_binary.SubclassDlgItem(IDC_BTN_BINARY,this);
	m_btn_binary.SetIcon(IDI_ICON_BINARY2);

	//标记人脸位置
	m_btn_mark_face.SubclassDlgItem(IDC_BTN_MARK_FACE_1,this);
	m_btn_mark_face.SetIcon(IDI_ICON_MARK_FACE2);

	//边缘选取
	m_btn_edge.SubclassDlgItem(IDC_BTN_EDGE,this);
	m_btn_edge.SetIcon(IDI_ICON_EDGE);

	//标记眼睛
	m_btn_mark_eye.SubclassDlgItem(IDC_BTN_MARK_EYE,this);
	m_btn_mark_eye.SetIcon(IDI_ICON_HALLOWEEN2,IDI_ICON_HALLOWEEN1);

	//标记嘴巴
	m_btn_mark_mouse.SubclassDlgItem(IDC_BTN_MARK_MOUSE,this);
	m_btn_mark_mouse.SetIcon(IDI_ICON_MARK_MOUSE2);

	//标记鼻子
	m_btn_mark_nose.SubclassDlgItem(IDC_BTN_MARK_NOSE,this);
	m_btn_mark_nose.SetIcon(IDI_ICON_MARK_NOSE2);
...
// 人脸评分
////////////////////////////////////////////////////////////////////////////////
void CFaceDetectDlg::CountEyeWidth()
{
	//标准一：计算双目宽度是否一致（满分为20分）
	//标准二：双眼距离则占脸宽的42%（满分为20分）
	//标准三：眼睛到嘴巴长度比例占脸长的33%（满分为40分）
	//标准四：符合“五眼”（满分为20分）
	
	//定义初始值
    float score=0;   //初始分数为0
	float WidthStd=0.42;    //标准二
	float HeightStd=0.33;   //标准三
	
	//标准一：
	int LeftEyeWidth = m_LeftEyeRightCorner.x - m_LeftEyeLeftCorner.x;
	int RightEyeWidth = m_RightEyeRightCorner.x - m_RightEyeLeftCorner.x;
	float diff = fabs(RightEyeWidth-LeftEyeWidth);
	float Std = (RightEyeWidth+LeftEyeWidth)/2;
	score += (float)(Std-diff)/Std*20;
	...
	EyesDis = (int)sqrt(EyesDis);
	//脸宽度
	int FaceWidth = m_rFaceRegion.right - m_rFaceRegion.left;
    float WidthRate = (float)EyesDis/(float)FaceWidth;
	//按比例获得标准二分数
	diff = fabs(WidthRate-WidthStd);
	score += (float)(WidthStd-diff)/WidthStd*20;
    
	//标准三：
	//眼睛到嘴巴的距离
	int EyeV    = (m_RightEye.y + m_LeftEye.y)/2;
	int EyesMouthDis = EyeV - m_MidMouth.y;
	//脸长度
	int FaceHeight = m_rFaceRegion.top - m_rFaceRegion.bottom;
    float HeightRate = (float)EyesMouthDis/(float)FaceHeight;
	//按比例获得标准二分数
	diff = fabs(HeightRate-HeightStd);
	score += (float)(HeightStd-diff)/HeightStd*40;

	//标准四
	float FiveStd = (float)FaceWidth/LeftEyeWidth;
	diff = fabs(FiveStd-5.0);
	score += (float)(5.0-diff)/5.0*20;
